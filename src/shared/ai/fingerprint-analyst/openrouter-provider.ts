import { defaultOpenRouterModelId, openRouterGenerateJson } from "../openrouter/client";
...
export class OpenRouterFingerprintAnalystProvider implements IFingerprintAnalystProvider {
  ...
  async infer(input: FingerprintAnalystInput): Promise<FingerprintAnalystResult> {
    ...
    if (mode !== "LIVE") {
      throw new Error("OpenRouterFingerprintAnalystProvider invoked outside LIVE provider mode");
    }

    const started = Date.now();
    const stageLabel = "OpenRouter fingerprint analyst";
    ...
    const { parsed } = await openRouterGenerateJson({
      apiKey: this.apiKey,
